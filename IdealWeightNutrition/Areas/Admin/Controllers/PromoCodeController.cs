    ...
    public class PromoCodeController : Controller
    {
        ...
        public IActionResult Create(PromoCode promoCode)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                promoCode.CreatedDate = IdealWeightNutrition.Utility.DateTimeHelper.Now;
            ...
            var allComboOffers = _unitOfWork.ComboOffer.GetAll(co => !co.IsDeleted && co.IsActive, includeProperties: "ComboOfferItems")
                ...
                .Select(co => new Microsoft.AspNetCore.Mvc.Rendering.SelectListItem
                {
                    Text = (currentCulture == "ar" && !string.IsNullOrEmpty(co.NameAr)) 
                        ? $"{co.NameAr} ({_localizer["ComboPrice"].Value}: {IdealWeightNutrition.Utility.CurrencyHelper.GetCurrencySymbol(currentCulture)} {co.ComboPrice:N2})" 
                        : $"{co.Name} ({_localizer["ComboPrice"].Value}: {IdealWeightNutrition.Utility.CurrencyHelper.GetCurrencySymbol(currentCulture)} {co.ComboPrice:N2})",
        ...
        public IActionResult GetComboOfferForDropdown(int comboOfferId)
        {
            ...
            var text = $"{comboOfferName} ({comboPriceText}: {currencySymbol} {comboOffer.ComboPrice:N2})";
