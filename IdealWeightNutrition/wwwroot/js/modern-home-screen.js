...
(function() {
    ...
    // Helper function to escape HTML entities (XSS protection)
    ...
    function fetchSearchSuggestions(term) {
        // Implement search suggestions using GetPaginatedProducts endpoint
        if (term.length < 2) {
            hideSearchSuggestions();
            return;
        }
        
        // Debounce search suggestions (using existing searchTimeout variable)
        if (searchTimeout) {
            clearTimeout(searchTimeout);
        }
        ...
        searchTimeout = setTimeout(async () => {
            try {
                // Use GetPaginatedProducts endpoint with limit of 5 suggestions
                const response = await fetch(`/Customer/Home/GetPaginatedProducts?page=1&pageSize=5&searchTerm=${encodeURIComponent(term)}`);
                ...
                if (data.products && data.products.length > 0 && searchSuggestions) {
                    const suggestionsHTML = data.products.map(product => {
                        const productSlug = escapeHtml(String(product.slugEn || product.id));
                        const imageUrl = escapeHtml(String(product.firstImageUrl || product.imageUrl || '/images/no-image.png'));
                        const title = (document.documentElement.lang === 'ar' && product.titleAr) ? product.titleAr : product.title;
                        const escapedTitle = escapeHtml(String(title || 'Product'));
                        const escapedPrice = escapeHtml(String(product.price || '0.00'));
                        ...
                        return `
                            <div class="search-suggestion-item" 
                                 role="button" 
                                 tabindex="0" 
                                 aria-label="View product: ${escapedTitle}, Price: ${escapedPrice} AED"
                                 onclick="performSearch('${escapedTerm.replace(/'/g, "\\'")}'); window.location.href='/Customer/Home/Details/${productSlug}';"
                                 onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();performSearch('${escapedTerm.replace(/'/g, "\\'")}'); window.location.href='/Customer/Home/Details/${productSlug}';}">
                                <img src="${imageUrl}" alt="${escapedTitle}" class="suggestion-image" onerror="this.src='/images/no-image.png'; this.onerror=null;" />
                                <div class="suggestion-info">
                                    <div class="suggestion-title">${escapedTitle}</div>
                                    <div class="suggestion-price">${escapedPrice} AED</div>
                                </div>
                            </div>
                        `;
                    }).join('');
                    ...
                    if (searchSuggestions) {
                        searchSuggestions.innerHTML = '<div class="search-suggestion-item no-results">No products found</div>';
                        showSearchSuggestions();
                    }
                }
            } catch (error) {
    ...
    let isLoading = false;
    let currentPage = 1; // Start at page 1 for new endpoint
    let currentFilters = {};
    let hasMoreProducts = false; // Track if there are more products to load
    ...
    function loadProductsWithFilters(filters, resetPage = false) {
        ...
        if (resetPage) {
            currentPage = 1; // Reset to first page (page 1, not 0)
        ...
        // USE NEW ENDPOINT: GetPaginatedProducts instead of LoadMoreProducts
        const url = `/Customer/Home/GetPaginatedProducts?page=${currentPage}&pageSize=20${filterParams ? '&' + filterParams : ''}`;
        ...
        fetch(url)
            ...
            .then(data => {
                if (data.products && data.products.length > 0) {
                    ...
                    // Update pagination state (NEW format from GetPaginatedProducts)
                    if (typeof paginationState !== 'undefined') {
                        paginationState.currentPage = data.currentPage || 1;
                        paginationState.totalPages = data.totalPages || 1;
                        paginationState.totalProducts = data.totalProducts || 0;
                    }
                    
                    // Update hasMoreProducts based on pagination data
                    const currentPageNum = data.currentPage || 1;
                    const totalPagesNum = data.totalPages || 1;
                    hasMoreProducts = currentPageNum < totalPagesNum;
                    
                    // Update pagination UI if function exists
                    if (typeof updatePaginationUI === 'function' && data.totalPages > 1) {
                    ...
                    // Update hasMoreProducts - no more products
    ...
    function renderProducts(products, isFirstPage) {
        ...
        // Track existing cards before adding new ones (for append mode)
        ...
        // Try to find createProductCardFromData function - check multiple possible locations
        let createProductCardFn = null;
        if (typeof window.createProductCardFromData === 'function') {
            createProductCardFn = window.createProductCardFromData;
        } else if (typeof createProductCardFromData === 'function') {
            createProductCardFn = createProductCardFromData;
        ...
        products.forEach((product, index) => {
            ...
            if (createProductCardFn) {
                ...
                // Fallback: createProductCardFromData function not found, using simple card
                if (typeof console !== 'undefined' && console.warn) {
                    console.warn('createProductCardFromData function not found, using fallback');
                }
                productHtml = createSimpleProductCard(product);
            }
    ...
    function createSimpleProductCard(product) {
        ...
        const productSlug = escapeHtml(String(product.slugEn || product.id));
        const imageUrl = escapeHtml(String(product.imageUrl || product.firstImageUrl || '/images/no-image.png'));
        const productTitle = escapeHtml(String(product.title || 'Product'));
        const productPrice = escapeHtml(String(product.price || '0.00'));
        
        return `
            <div class="product-card modern-stagger-item visible" 
                 role="button" 
                 tabindex="0" 
                 aria-label="View product: ${productTitle}, Price: ${productPrice}"
                 onclick="window.location.href='/Customer/Home/Details/${productSlug}'"
                 onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();window.location.href='/Customer/Home/Details/${productSlug}';}">
                <div class="product-image-wrapper">
                    <img src="${imageUrl}" class="product-image" alt="${productTitle}" onerror="this.src='/images/no-image.png'; this.onerror=null;" />
                </div>
    ...
    // OLD Load More button functions removed - using pagination instead!
    // Pagination is now handled by the main page's loadPage() function
    ...
    function loadMoreProducts() {
        // Load next page if there are more products
        if (hasMoreProducts && !isLoading) {
            currentPage++;
            loadProductsWithFilters({}, false); // Don't reset, just append
        }
    }
