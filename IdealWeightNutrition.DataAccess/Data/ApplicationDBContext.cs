...
namespace IdealWeightNutrition.DataAccess.Data
{
    public class ApplicationDBContext:IdentityDbContext<IdentityUser>
    {
		...
		protected override void OnModelCreating(ModelBuilder modelBuilder)
		{
			...
			// OrderAuditLog indexes - For audit trail queries
			modelBuilder.Entity<OrderAuditLog>().HasIndex(oal => oal.OrderHeaderId); // For order audit history
			modelBuilder.Entity<OrderAuditLog>().HasIndex(oal => new { oal.OrderHeaderId, oal.ActionDate }); // For ordered audit logs
			modelBuilder.Entity<OrderAuditLog>().HasIndex(oal => oal.PerformedByUserId); // For user actions
			...
			// ==========================================
			// PHASE 1 PERFORMANCE: Search & Filter Indexes
			// ==========================================
			// Indexes for search fields (Title, TitleAr) - CRITICAL for search performance
			// Note: These help with prefix searches and exact matches
			// Full-text search will be added separately via SQL migration
			modelBuilder.Entity<Product>().HasIndex(p => p.Title).HasDatabaseName("IX_Products_Title");
			modelBuilder.Entity<Product>().HasIndex(p => p.TitleAr).HasDatabaseName("IX_Products_TitleAr");
			
			// Indexes for product status filters (IsNew, IsTrending)
			modelBuilder.Entity<Product>().HasIndex(p => new { p.IsDeleted, p.IsNew }).HasDatabaseName("IX_Products_IsDeleted_IsNew");
			modelBuilder.Entity<Product>().HasIndex(p => new { p.IsDeleted, p.IsTrending }).HasDatabaseName("IX_Products_IsDeleted_IsTrending");
			modelBuilder.Entity<Product>().HasIndex(p => new { p.IsDeleted, p.IsNew, p.IsTrending }).HasDatabaseName("IX_Products_IsDeleted_IsNew_IsTrending");
			
			// Composite index for common search + filter combinations
			modelBuilder.Entity<Product>().HasIndex(p => new { p.IsDeleted, p.CategryId, p.Price })
				.HasDatabaseName("IX_Products_IsDeleted_CategryId_Price_Composite");
			
			// FlashSaleItem indexes for product/variant lookups
